...
def _parse_score(response) -> float | None:
    """Return a float in [0,1], or None if the judge failed. Never 0.0 on error."""
    ...
    if not nums:
        print(f"  !! judge returned no number: {text[:200]!r}")
        return None
    val = float(nums[-1])          # last number — reasoning models trail the answer
...
def run_evaluation():
    ...
    print("\nSaved: test_results.json + test_summary.json")
